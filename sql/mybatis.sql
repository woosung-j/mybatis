...
create table mybatis.addresses (
    user_id number(2),
    address varchar2(30),
    constraint addr_userid_pk primary key(user_id),
    constraint addr_userid_fk foreign key(user_id) 
...
create table mybatis.posts (
    ...
    constraint post_userid_fk foreign key(user_id) 
                              references mybatis.users(user_id)
);
...
alter table mybatis.addresses
    add constraint addr_userid_fk 
        foreign key(user_id) 
        references mybatis.users(user_id);

alter table mybatis.posts
    add constraint post_use_id_fk
        foreign key(user_id)
        references mybatis.users(user_id);
*/
